...
function reformat(slots: Slots<CodeSlotSlotLiteral, CodeSlot>, startSlot: CodeSlot, languageGrammar: Grammar, commentStart: string) {
  const list = slots.toArray()
  let i = list.indexOf(startSlot)
  let startBlock = startSlot.isBlockComment
  for (; i < list.length; i++) {
    const slot = list[i]
    let code = slot.sliceContent()[0] as string
    slot.isBlockComment = startBlock
    if (startBlock) {
      code = commentStart + code
    }
    ...
    startBlock = !!lastToken && typeof lastToken !== 'string' &&
      ...
      (lastToken.content as string).indexOf(commentStart) === 0

    const next = list[i + 1]
    if (next && next.isBlockComment === startBlock) {
      break
    }
  }
}
...
export class CodeSlot extends Slot {
  ...
  override toJSON(): CodeSlotSlotLiteral {
    return {
      ...super.toJSON(),
      isBlockComment: this.isBlockComment
    }
  }
}
...
export const preComponent = defineComponent({
  ...
  setup(data: PreComponentState) {
    ...
    let [blockCommentStart] = getLanguageBlockCommentStart(data.lang)
    ...
    stateController.onChange.subscribe(newLang => {
      ...
      blockCommentStart = getLanguageBlockCommentStart(newLang)[0]
      isStop = true
      slots.toArray().forEach(i => i.isBlockComment = true)
      slots.get(0)!.isBlockComment = false
      reformat(slots, slots.get(0)!, languageGrammar!, blockCommentStart)
      isStop = false
    })

    let startBlock = false
    const slots = useSlots<CodeSlot, CodeSlotSlotLiteral>(data.code.split('\n').map(i => {
      const slot = new CodeSlot()
      slot.isBlockComment = startBlock
      if (startBlock) {
        i = blockCommentStart + i
      }
      slot.insert(i)
      if (languageGrammar) {
      ...
      s.isBlockComment = state.isBlockComment
      return s
    })
    ...
    slots.onChildSlotChange.subscribe(data => {
      if (languageGrammar && !isStop) {
        ...
        reformat(slots, data.source, languageGrammar, blockCommentStart)
    ...
    onPaste(ev => {
      ...
      if (firstCode) {
        ev.target.insert(firstCode)
      }
      const index = slots.indexOf(ev.target as CodeSlot)
      slots.retain(index + 1)
      slots.insert(...codeList.map(i => {
        const slot = new CodeSlot()
        slot.insert(i)
        return slot
      }))
