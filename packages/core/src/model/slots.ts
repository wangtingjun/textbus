...
export class Slots<SlotState extends SlotLiteral = SlotLiteral, T extends Slot = Slot> {
  ...
  insertAfter(slot: T, ref: T) {
    const index = this.slots.indexOf(ref)
    if (index > -1) {
      this.insertByIndex(slot, index + 1)
    }
  }

  insertBefore(slot: T, ref: T) {
    const index = this.slots.indexOf(ref)
    if (index > -1) {
      this.insertByIndex(slot, index)
    }
  }

  insertByIndex(slot: T, index: number) {
    if (index < 0) {
      index = 0
    }
  ...
  insert(...slots: T[]) {
    ...
    slots.forEach(i => {
      ...
      i.parent = this.host
      this.changeListeners.set(i, i.changeMarker.onChange.subscribe(operation => {
