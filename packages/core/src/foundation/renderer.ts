...
export class Renderer {
  ...
  private diffNodeAndUpdate(newVDom: VElement, oldVDom: VElement) {
    ...
    if (oldVDom.tagName !== newVDom.tagName) {
      ...
      const {styleChanges, attrChanges, classesChanges} = getNodeChanges(newVDom, oldVDom)
  ...
  private createVDomByFormatTree(slot: Slot, formats: FormatTree[]) {
    ...
    formats.map(child => {
      if (child.formats) {
        ...
        const formats = Renderer.formatSort(child.formats)
  ...
  private static formatSort(formats: FormatItem[]) {
    formats.sort((a, b) => {
